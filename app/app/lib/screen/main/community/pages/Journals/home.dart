...
import 'package:hikup/screen/main/community/sharedPreferences.dart';
...
class _HomePageState extends State<ComePage> {
  ...
  taskUpdation(int index) async{
    ...
    if(allChecklists!=null){
      ...
      fToast.init(context);
      showToast(fToast, "Task1 added to the task list successfully ", NotificationStatus.success);
    }
  }
  ...
  submitCheckLists()async{
    if(addChecklistController.text.isEmpty){
      fToast.init(context);
      showToast(fToast, "Cette section ne peut pas etre vide", NotificationStatus.failure);
    }else{
      String? allChecklists  = await sharedPreferences.getFromSharedPref('all-checklist');
      if (allChecklists!=null) {
        ...
        fToast.init(context);
        showToast(fToast, "Task3 added to the task list successfully ", NotificationStatus.success);
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Color.fromARGB(255, 206, 218, 207),
      appBar: AppBar(
        elevation: 0,
      ...
      child:SingleChildScrollView(
          child:Column(
            ...
            GestureDetector(
              ...
              child: Container(
              padding: const EdgeInsets.only(left:20,top: 3, bottom: 10, right: 20),
              child:Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                
                const Text("Toutes les demandes",
    ...
    Container(
        ...
        Container(
          ...
          child:Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
            const Text("Liste des évenements",
              style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color.fromARGB(255, 52, 63, 71)),
              ),
              Row(
                children: [
                GestureDetector(child:Text("Tous voir",
              style: TextStyle(fontSize: 17, fontWeight: FontWeight.bold,color: Color.fromARGB(255, 86, 127, 160)),
              ),
              onTap: (){
                Navigator.push(
        ...
        FutureBuilder(
            ...
            builder:(context, AsyncSnapshot snapshot) {
              if (snapshot.hasError) {
                ...
                return const Center(
                    child: Text("pas d'événements crée"));
              } else {
                return Container(
      ...
      floatingActionButton: FloatingActionButton.extended(
        backgroundColor: Color.fromARGB(255, 14, 191, 41),
        onPressed: (){
          Navigator.push(
            context,
          ...
          label:Row(children: const [Icon(Icons.add),SizedBox(width: 5,),Text("Inviter une personne",style: TextStyle(color: Color.fromARGB(255, 255, 255, 255)),)],)
      ),
    );
  }
}